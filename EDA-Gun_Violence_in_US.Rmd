...
```{r}
totalCasualties <- gun %>% group_by(year = format(date, "%Y")) %>% summarize(casualties = sum(n_killed + n_injured), fatalities = sum(n_killed), injuries = sum(n_injured))
...
Let's now look at the *cities* with the highest rates of per capita gun violence. I again used data from the [U.S. Census Bureau][1] to find the population of cities in the United States. The dataset I used contained data from 2010-2016 for all cities greater than 50,000 residents. I had to do some manipulation of the city names in order to make them match the cities in the original dataset. Also, since there are no data for 2017, I just used the data for 2016 and assumed there wouldn't be any significant changes.
